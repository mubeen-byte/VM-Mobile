...
const TYPE = 'handover-to-live-agent';
...
async function getHandoverTicketsDailyRows(client, startMs, endMs) {
  const sql = `
    WITH bounds AS (
      SELECT
    ...
    handover AS (
      ...
      WHERE "Created" BETWEEN $1 AND $2
        ...
        AND "a.escalated" IS TRUE
    ),
    joined AS (
      SELECT
        to_char(days.day, 'YYYY-MM-DD') AS day_str,
        handover.ticket_id AS ticket_id
      FROM days
      LEFT JOIN handover
        ON handover.day = days.day
    )
    SELECT
      day_str AS day,
...
exports.handler = async () => {
  ...
  try {
    const rows = await getHandoverTicketsDailyRows(clientAS, startMs, endMs);
